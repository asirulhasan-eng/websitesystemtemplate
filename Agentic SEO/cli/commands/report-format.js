/**
 * report-format.js Ã¢â‚¬â€ Format arbitrary data into report templates
 *
...
const HELP = `
report-format Ã¢â‚¬â€ Format arbitrary data into report templates
...
TEMPLATES
  daily   Ã¢â‚¬â€ Formatted daily activity summary with task/GSC/SERP sections
  weekly  Ã¢â‚¬â€ Week-over-week comparison with daily breakdown table
  alert   Ã¢â‚¬â€ Urgent alert with severity, message, and recommended actions
  workplan Ã¢â‚¬â€ Reviewable 12-hour work plan email (opt-out via Telegram)
  custom  Ã¢â‚¬â€ Generic key-value rendering with optional custom title
`.trim();
...
function renderAlert(data) {
  const d = data || {};
  const lines = [
    `# Ã¢Å¡Â  Alert: ${d.severity || 'WARNING'}`,
    '',
...
function renderCustom(data, title) {
  ...
  lines.push('', '---', `*Generated by Website Operations Agent*`);
  return lines.join('\n');
}
...
function renderWorkplan(data) {
  ...
  const healthIcon = health === 'green' ? 'Ã°Å¸Å¸Â¢' : health === 'yellow' ? 'Ã°Å¸Å¸Â¡' : health === 'red' ? 'Ã°Å¸â€Â´' : 'Ã¢Å¡Âª';
  ...
  const laneLabel = {
    safe: 'Safe Ã¢â‚¬â€ auto-applied & deployed to production',
    semi: 'Semi-safe Ã¢â‚¬â€ built on a preview branch for your review',
    semi_safe: 'Semi-safe Ã¢â‚¬â€ built on a preview branch for your review',
    high: 'High-risk Ã¢â‚¬â€ reversible, will run unless you stop it',
    high_risk: 'High-risk Ã¢â‚¬â€ reversible, will run unless you stop it',
  };
  const laneOrder = ['safe', 'semi', 'semi_safe', 'high', 'high_risk'];

  const lines = [];
  lines.push(`# ${healthIcon} Website Operations Work Plan Ã¢â‚¬â€ ${session}`);
  lines.push('');
  ...
  lines.push('## Ã¢Å“â€¦ How to review');
  ...
  if (planned.length === 0) {
    ...
    for (const lane of seenLanes) {
      ...
      for (const t of byLane[lane]) {
        lines.push(`### ${t.id ? `${t.id} Ã¢â‚¬â€ ` : ''}${t.title || 'Untitled task'}`);
  ...
  if (needsGo.length > 0) {
    lines.push('## Ã¢â€ºâ€ Needs your explicit go-ahead (will NOT auto-run)');
  ...
  if (alerts.length > 0) {
    lines.push('## Ã°Å¸â€œâ€° Ranking alerts');
  ...
  lines.push('---');
  lines.push(`*Generated ${nowIso()} Ã‚Â· Website Operations Agent Ã‚Â· reply on Telegram to intervene*`);
  return lines.join('\n');
}

function markdownToHtml(md) {
  // Minimal markdown Ã¢â€ â€™ HTML conversion
  let html = md
    .replace(/^# (.+)$/gm, '<h1>$1</h1>')
    .replace(/^## (.+)$/gm, '<h2>$1</h2>')
    .replace(/^### (.+)$/gm, '<h3>$1</h3>')
  ...
  return `<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>Website Operations Report</title>
