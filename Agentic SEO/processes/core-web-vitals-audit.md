...
trigger:
  schedule: "0 3 1 * *"            # Monthly, 1st of month, 03:00 BST
  timezone: "Asia/Dhaka"
...
> turn the worst offenders into concrete fix tasks. Slow pages lose rankings **and** conversions Ã¢â‚¬â€
...
|--------|------|-------------------|------|------------------|
| **LCP** Ã¢â‚¬â€ Largest Contentful Paint | < 2.5s | 2.5Ã¢â‚¬â€œ4s | Ã¢â€°Â¥ 4s | When the main content renders |
| **CLS** Ã¢â‚¬â€ Cumulative Layout Shift | < 0.1 | 0.1Ã¢â‚¬â€œ0.25 | Ã¢â€°Â¥ 0.25 | Visual stability (unexpected movement) |
| **INP** Ã¢â‚¬â€ Interaction to Next Paint | < 200ms | 200Ã¢â‚¬â€œ500ms | Ã¢â€°Â¥ 500ms | Responsiveness to user input |
| **TTFB** Ã¢â‚¬â€ Time to First Byte | < 0.8s | 0.8Ã¢â‚¬â€œ1.8s | Ã¢â€°Â¥ 1.8s | Server/caching responsiveness (LCP precursor) |

> Prefer **field data (CrUX)** over lab data when available Ã¢â‚¬â€ it reflects real users. Use lab
...
1. Confirm a `PAGESPEED_API_KEY` (or `GOOGLE_API_KEY`) is set Ã¢â‚¬â€ anonymous PSI rate limits are low and
   will throttle a multi-URL run. The tool runs without a key but may fail under `retry_policy`.
2. Decide the URL set: every money/service template + the homepage + 1Ã¢â‚¬â€œ2 representative blog posts.
   You do not need to test every URL Ã¢â‚¬â€ test one of each **template** plus the high-value pages.
...
# Money / service pages (mobile-first Ã¢â‚¬â€ Google indexes mobile)
v2 speed-audit --url https://example.com/services/website-seo/ --strategy mobile --json
v2 speed-audit --url https://example.com/services/seo-for-small business owners/ --strategy mobile --json

# A representative blog template
v2 speed-audit --url https://example.com/blog/<slug>/ --strategy mobile --json
```
...
> **Async note.** Each run takes ~30Ã¢â‚¬â€œ90s. If a run times out, do not block the whole audit Ã¢â‚¬â€
...
- Assign explicit `width`/`height` (or `aspect-ratio`) to all images and media slots Ã¢â€ â€™ kills CLS.
...
v2 task create --title "Perf: Improve LCP on homepage hero (3.9s mobile)" \
  ...
  --evidence '{"source":"pagespeed","strategy":"mobile","url":"https://example.com/","lcp_ms":3900,"cls":0.04,"field_category":"NEEDS_IMPROVEMENT","top_opportunity":"properly-size-images"}' \
  --json

v2 task create --title "Perf: Eliminate layout shift on /services/website-seo/ (CLS 0.21)" \
  --type technical_fix --priority 700 --risk-level semi_safe \
  --target-file "services/website-seo/index.html" \
  ...
  --evidence '{"source":"pagespeed","strategy":"mobile","url":"https://example.com/services/website-seo/","cls":0.21,"field_category":"NEEDS_IMPROVEMENT"}' \
  --json
```
