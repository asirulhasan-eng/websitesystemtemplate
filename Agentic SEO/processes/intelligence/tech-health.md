...
> 1st of the month: is the site technically sound Ã¢â‚¬â€ fast, indexed, crawlable?
...
## AI Analysis
- **Core Web Vitals** Ã¢â‚¬â€ LCP / INP / CLS regressions vs prior; pages failing thresholds.
- **Indexing** Ã¢â‚¬â€ pages dropped from the index, "discovered not indexed", coverage errors.
- **Crawl** Ã¢â‚¬â€ crawl errors, redirect chains, crawl-budget waste (low-value URLs being crawled).
- **Sitemap** Ã¢â‚¬â€ invalid/missing/orphan URLs in the sitemap.
- **Manual actions / security** Ã¢â‚¬â€ any GSC manual action or security issue (escalate hard if present).

Recall Brain memory for known technical issues (`brain recall --query "core web vitals"`).

Severity:
- `critical` Ã¢â‚¬â€ manual action / security issue, or money pages deindexed.
- `warning` Ã¢â‚¬â€ CWV regressions or indexing/crawl problems on important pages.
- `normal` Ã¢â‚¬â€ clean.
...
node $V2 intelligence report \
  ...
  --report-json '{
    "threats":[{"type":"serp_change","keyword":"<url>","severity":"warning","recommendation":"LCP 4.1s Ã¢â‚¬â€ investigate render-blocking resources"}],
    "observations":["No manual actions","Sitemap valid, 2 orphan URLs"],
    "recommendations":[{"priority":"high","action":"Planner: queue a cwv_fix task (risk=safe, general_operational lane â€” the safe executor lazy-loads below-fold images and defers render-blocking head scripts deterministically) for <pages>","evidence":"LCP > 2.5s on 3 service pages"}],
