...
> version Ã¢â‚¬â€ the full investigation lives in `processes/content-gap-analysis.md`
...
- **Wrong-page rankings** Ã¢â‚¬â€ a query ranks via a page that isn't the best fit (e.g. a blog post
  ranking for a commercial term that a service page should own).
- **Missing pages** Ã¢â‚¬â€ clusters of impressions with no good landing page at all.
- **Cannibalization** Ã¢â‚¬â€ two+ site pages competing for the same query (split signals, neither
  ranks well).
...
### Page-Fit Assessment (WS3 Ã¢â‚¬â€ for every money-keyword cluster)
...
3. **Classify the ranking URL type** using the content inventory:
   - `/services/*` Ã¢â€ â€™ service page
   - `/blog/*` Ã¢â€ â€™ blog post
   - `/` or homepage Ã¢â€ â€™ homepage
   - Other Ã¢â€ â€™ utility/other
4. **Determine page_fit verdict:**
   - `canonical` Ã¢â‚¬â€ the ranking URL IS the expected page type (e.g., commercial keyword Ã¢â€ â€™ service page) Ã¢Å“â€¦
   - `wrong_page` Ã¢â‚¬â€ a commercial/money keyword ranks through a blog post or homepage, but should have a dedicated service page Ã¢Å¡Â Ã¯Â¸Â
   - `missing` Ã¢â‚¬â€ no page ranks for this cluster at all, or only ranks via unrelated pages Ã°Å¸â€Â´

**CRITICAL: Remove position bias.** A money cluster ranking via a blog at **ANY** position is a
wrong-page candidate. Weight by `total_impressions Ãƒâ€” intent_tier`, NOT by position Ã¢â€°Â¤ 15.
...
```json
{
  ...
  "current_url": "/blog/website-seo-audit-process",
...
node $V2 intelligence report \
  ...
  --report-json '{
    ...
    "observations":["2 pages cannibalizing 'emergency small business owners seo'"],
