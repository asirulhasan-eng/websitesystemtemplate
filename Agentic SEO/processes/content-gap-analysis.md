...
**Examples:**
- "website SEO pricing" lands on the homepage instead of `/services/pricing/`
- "local SEO for small business owners" lands on a generic blog post instead of `/services/local-seo-for-small business owners/`
- "small business owners website design" lands on the SEO service page instead of a dedicated web design page
...
**Examples:**
- Impressions for "small business owners reputation management" but no page covers this topic
- Queries about "small business owners social media marketing" but we have no content on this
- "website owner Google Ads" generating impressions Ã¢â‚¬â€ adjacent service we don't cover yet
...
**Examples:**
- Both `/services/website-seo/` and `/blog/what-is-website-seo/` rank for "website SEO"
- Multiple blog posts all targeting "small business owners marketing ideas"
...
v2 page-read --url "https://example.com/services/pricing/" --json
v2 page-meta --url "https://example.com/services/pricing/" --json
```
...
v2 serp-check --keywords "website SEO pricing" --domain example.com --json
```
...
v2 task-search --keyword "website SEO pricing" --days 90 --json
v2 gsc-history --keyword "website SEO pricing" --days 90 --json
```

---

## Step 3: AI Analysis Ã¢â‚¬â€ Diagnose the Gap
...
> (guides), forcing your service page to rank is wasted effort Ã¢â‚¬â€ and vice versa.

**Step 3.0 Ã¢â‚¬â€ Top-10 intent distribution (run this first):**
...
- **If the dominant intent matches the page you want to rank** (e.g. mostly service pages, and you want your service page to rank) Ã¢â€ â€™ proceed with the structural fix below.
- **If Google now prefers a different content type** (e.g. mostly guides, but you're pushing a service page) Ã¢â€ â€™ do **not** force the service page. Either create/optimize a page of the matching type, or expand the service page to genuinely satisfy the observed intent. Record this as an intent-shift finding, not a wrong-page bug.
...
2. **Should this be a new service page or a blog post?**
   ...
   **Create a SERVICE PAGE when:**
   - The keyword has commercial/transactional intent ("small business owners SEO services", "website SEO pricing")
   ...
   **Create a BLOG POST when:**
   - The keyword has informational intent ("how to get more website customers")
...
3. **Does this create a content hub opportunity?**
   ...
   - Example: queries about "small business owners Google Business Profile", "small business owners Google Maps", "small business owners local pack" Ã¢â€ â€™ create a Local SEO for small business owners hub page with supporting articles
...
2. **How bad is the cannibalization?** Check if:
   - Both pages are on page 1 (less urgent Ã¢â‚¬â€ could even be beneficial)
   - Both pages are on page 2+ (likely hurting Ã¢â‚¬â€ Google is confused)
   - Positions are fluctuating between the pages (Google is testing Ã¢â‚¬â€ needs resolution)
...
> Do **not** add repeated exact-match keyword anchors across many pages Ã¢â‚¬â€ done programmatically
...
>   (e.g. for "website SEO pricing": "our pricing packages", "what website SEO costs",
>   "see pricing details" Ã¢â‚¬â€ not five identical "website SEO pricing" links).
...
### Response Plan: No Page Exists Ã¢â‚¬â€ New Service Page

```
PLAN:
1. Create a comprehensive service page under /services/
2. Target primary keyword + 3-5 related secondary keywords
3. Include: clear service description, benefits for website owners, process/methodology, pricing signals, testimonials, CTA
...
### Response Plan: No Page Exists Ã¢â‚¬â€ New Blog Post
...
4. Focus on informational depth and practical value for website owner owners.
...
4. Update internal linking to consistently point to the canonical page (contextual anchors Ã¢â‚¬â€ see
   the anchor-text rule above)
5. If merging: 301 redirect the deprecated page Ã¢â€ â€™ canonical page, then run the redirect-cleanup
   checklist below
...
>
> 1. **Create the 301** from the old URL Ã¢â€ â€™ final canonical URL (one hop, never a chain).
...
> 5. **Check for redirect chains** (AÃ¢â€ â€™BÃ¢â€ â€™C). Collapse to a single hop (AÃ¢â€ â€™C).
> 6. **Re-crawl** the affected URLs to verify status codes and that no internal link still 301s.
> 7. **Submit the old URL for re-indexing in GSC** (URL Inspection Ã¢â€ â€™ Request Indexing) so Google
...
```bash
v2 task create \
  --title "Content Gap Fix: 'website SEO pricing' ranking on homepage instead of /services/pricing/" \
  --type content_optimization \
  --priority 800 \
  --risk-level semi_safe \
  --target-url "https://example.com/services/pricing/" \
  --target-keyword "website SEO pricing" \
  --description "Wrong page ranking: homepage ranks for 'website SEO pricing' at position 14 instead of the dedicated pricing page. Plan: optimize pricing page title/H1/content for this keyword, add internal links from homepage to pricing page using keyword anchor, update sitewide internal linking. Expected result: pricing page replaces homepage in SERP with higher position due to better relevance match." \
  --evidence "GSC 28d: 'website SEO pricing' Ã¢â€ â€™ homepage at pos 14, 85 imp, 2 clicks. Pricing page not appearing for this query at all. SERP shows dedicated pricing/packages pages ranking in top 5." \
  --json
```
...
```bash
v2 task create \
  --title "Content Gap: Create service page for 'small business owners reputation management'" \
  --type new_content \
  --priority 500 \
  --risk-level safe \
  --target-keyword "small business owners reputation management" \
  --description "No page exists for reputation management queries. GSC shows growing impressions (60/week, up 25% month-over-month). SERP shows mostly service pages and guides. Recommend: create /services/reputation-management/ page covering review generation, review response, reputation monitoring for website owners. This supports the broader SEO service offering." \
  --evidence "GSC 28d: 'small business owners reputation management' at pos 22 (homepage), 60 imp/week trending up. Related queries also showing: 'small business owners reviews SEO' (30 imp), 'website owner reviews' (45 imp)." \
  --json
```
...
```bash
v2 task create \
  --title "Cannibalization Fix: /services/website-seo/ vs /blog/what-is-website-seo/ for 'website SEO'" \
  --type content_optimization \
  --priority 800 \
  --risk-level semi_safe \
  --target-url "https://example.com/services/website-seo/" \
  --target-keyword "website SEO" \
  --description "Two pages competing for 'website SEO': service page (pos 8, 200 imp) and blog post (pos 15, 80 imp). Service page should be canonical Ã¢â‚¬â€ it's the conversion page and matches SERP intent (mostly service pages in top 5). Plan: (1) update blog post to target 'what is website SEO' and 'website SEO explained' instead, (2) add clear internal link from blog Ã¢â€ â€™ service page, (3) ensure service page has strongest keyword signals." \
  --evidence "GSC 28d: Service page pos 8, 200 imp, 12 clicks. Blog post pos 15, 80 imp, 2 clicks. Both pages fluctuating in position Ã¢â‚¬â€ Google is testing." \
  --json
```
...
2. If >3 content gaps were found in a single scan, flag for strategic review Ã¢â‚¬â€ this may indicate a broader content strategy issue
...
| Situation | Action |
|-----------|--------|
| Money keyword on wrong page | Fix immediately Ã¢â‚¬â€ high priority task |
| Money keyword with no page | Create service page Ã¢â‚¬â€ high priority |
| Authority keyword on wrong page | Fix if >50 impressions Ã¢â‚¬â€ medium priority |
| Authority keyword with no page | Create blog post or expand existing page Ã¢â‚¬â€ medium priority |
| Informational keyword gap | Only act if >100 impressions or supports money keyword |
| Cannibalization on money keyword | Fix immediately Ã¢â‚¬â€ high priority |
