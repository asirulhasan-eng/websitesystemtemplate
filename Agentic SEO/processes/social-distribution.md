...
guardrails:
  ...
  notes: "Social reshares of already-approved, already-public content are semi-safe. Do NOT enqueue for preview-only or unapproved drafts. The pipeline never posts faster than 1/platform/floor and drains on a ~90Â±11 min cron, so there is no burst risk."
metadata:
  hermes:
    tags: [SEO, Website Operations, Social, Distribution, Content]
...
a post is approved and live â€” never for preview-only drafts.
...
2. **The drip cron (always-on):** `*/15 * * * * v2 social send --json` drains the
   pipeline at a jittered ~90 Â± 11 min cadence. Do **not** run `social send` in a
   loop yourself.

---

## âš ï¸ Caption rules â€” captions MUST be unique per platform
...
- **Facebook**: long, natural-language, keyword-rich. FB posts can rank in Google
  for long-tail phrasing â€” write a full, readable paragraph and include the blog URL.
- **Instagram**: long and keyword-rich too, but framed for IG (hook first line,
  hashtags at the end). Must DIFFER from the Facebook caption â€” different opening,
  different emphasis â€” not a duplicate.
...
- **X/Twitter**: one post for the whole blog, â‰¤280 chars, matching/hero image.
...
across infographics â€” **treat those warnings as a fail and rewrite.**

Character limits are enforced (X 280 Â· IG 2200 Â· Pinterest 500 Â· LinkedIn 3000 Â·
Reddit 40000 Â· FB 63206). Over-limit captions are rejected unless `--truncate`.
...
## Cadence model (configurable â€” not hardcoded)

- `each` platforms (default: facebook, instagram, pinterest) â†’ one post **per
  infographic**.
- `once` platforms (default: linkedin, twitter, reddit, youtube) â†’ one post **per
...
+ LinkedIn + X (+ Reddit); batch 1â€“2 = FB+IG+Pin only.
...
```json
{
  "blog_url": "https://example.com/blog/<slug>",
  "channels": ["facebook","instagram","pinterest","linkedin","x"],
  "infographics": [
    { "image_url": "https://example.com/assets/images/blog/<img1>.png",
      "captions": { "facebook": "â€¦", "instagram": "â€¦", "pinterest": "â€¦" } },
    { "image_url": "https://example.com/assets/images/blog/<img2>.png",
      "captions": { "facebook": "â€¦", "instagram": "â€¦", "pinterest": "â€¦" } }
  ],
  "linkedin": { "image_url": "https://example.com/assets/images/blog/<hero>.png", "caption": "â€¦" },
  "x":        { "image_url": "https://example.com/assets/images/blog/<hero>.png", "caption": "â€¦" }
}
```
...
this â€” the drip cron sends them.
