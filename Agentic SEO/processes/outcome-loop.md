# Outcome Loop â€” clicks-primary measurement + debounced lever-split remediation

> Closes the business-outcome edge of the autopilot. Every ranking-affecting change is
> measured for whether it actually helped â€” judged on **GSC clicks**, not just SERP
> position â€” and a change that **confirmed-regresses** is remediated automatically with
...
- **Baseline** clicks to the target URL over the `window_days` window are captured at
  deploy (`followups.captureBaselineClicks` â†’ `gsc_snapshots`) and stored on the
  follow-up + experiment.
- At the check, current clicks over the same window are compared.
  `outcome_loop.evaluateOutcome`:
  - **regressed** if clicks fell by â‰¥ `click_drop_ratio` (default 30%).
  - **improved** if clicks rose by â‰¥ `click_improve_ratio` (default 10%); else **stable**.
...
## Never act on a single dip â€” debounced confirmation
...
|---|---|---|
| not degraded | 0 | **success** â†’ close `improved`/`stable` |
| not degraded | â‰¥1 | **recovered** â†’ close `recovered_transient`, no action |
| degraded | < `required_consecutive_degraded`âˆ’1, within window | **watch** â†’ schedule a re-check in `recheck_days`, carry the counter |
| degraded | confirms `required_consecutive_degraded` | **act** â†’ remediate (below) |
| degraded | unconfirmed, past `max_watch_days` | **inconclusive** â†’ alert owner, no auto-action |
...
The 14-day attribution/`research_hold` window is **not** extended by the watch â€” it is
...
- **content** lever â†’ **auto-refresh**: enqueue a `content_refresh` task
  (`blog_content` lane â†’ `edit_refresh_needed` â†’ the `content-refresh` skill, in place).
  Reverting a published article is destructive, so content is re-optimized, never rolled back.
- **title / meta / canonical / image_alt / schema / internal_links** â†’ **auto-rollback**:
  resolve the offending commit from `deployments` (latest `commit_sha` for the parent
  change task) and `deploy-rollback --apply --push`.
- **corrective / unknown** lever â†’ **none** (a `ranking_recovery` task is filed for a human).
...
type, so it opens no new experiment â€” no rollback loop.
...
`config/guardrails.json` â†’ `outcome_loop` (every value env-overridable):
...
`auto_rollback_levers`, `auto_refresh_levers`. Env: `WEBSITE_AGENT_CLICK_DROP_RATIO`,
`WEBSITE_AGENT_MIN_BASELINE_CLICKS`, `WEBSITE_AGENT_FOLLOWUP_DAYS`, `WEBSITE_AGENT_RECHECK_DAYS`,
`WEBSITE_AGENT_REQUIRED_CONSECUTIVE_DEGRADED`, `WEBSITE_AGENT_MAX_WATCH_DAYS`.

These auto-remediation edges are **sanctioned** â€” the self-evaluation auditor should not
