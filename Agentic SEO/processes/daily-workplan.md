...
name: "Daily Planner"
version: 2
description: "Twice-daily planner. Reads the intelligence summary (from the intelligence pipeline) and the feedback brief (from the analyst), then PLANS and ENQUEUES tasks for the next 12 hours. It does NOT gather data or run analysis Ã¢â‚¬â€ the intelligence pipeline does that. It is the SOLE PRODUCER of tasks."
trigger:
  schedule: "0 2 * * *"            # Morning session Ã¢â‚¬â€ 8:00 AM BST
  schedule_evening: "0 14 * * *"   # Evening session Ã¢â‚¬â€ 8:00 PM BST
...
guardrails:
  ...
  require_explicit_approval_for:    # irreversible/destructive only Ã¢â‚¬â€ see config/guardrails.json
    - delete_page
    - domain_change
    - ssl_change
    - dns_change
...
> It does **NOT** touch the GSC API, the SERP API, or any raw data Ã¢â‚¬â€ the intelligence
> pipeline (`cron/run-intelligence.sh`, 30 min earlier) already did that and saved reports.

## Where this sits (three intelligence layers Ã¢â‚¬â€ keep them distinct)
...
| **This planner** | here (2Ãƒâ€”/day) | the plan + tasks | **Yes Ã¢â‚¬â€ only here** |
...
## Execution Model Ã¢â‚¬â€ enqueue-only producer, opt-out approval
...
- **Enqueue what you want run.** `v2 task update --id <id> --status approved`. The worker computes
  the lane (`v2 task next`) and dispatches it. **Everything deploys STRAIGHT TO PRODUCTION Ã¢â‚¬â€ there
  ...
  - **Deterministic edits** (internal links, meta/title/canonical/alt, schema, protect-ranking):
    safeÃ¢â€ â€™`safe-fix --apply --production`, semi_safeÃ¢â€ â€™`safe-fix --allow-semi-safe --apply --production`.
  - **Content tasks that REWRITE/AUTHOR page copy** Ã¢â‚¬â€ `new_blog_post`, `content_refresh`,
    `new_service_page`, **`service_page_gap`**, **`money_page_refresh`** Ã¢â‚¬â€ route to the `blog_content`
...
- **Legacy blog preview states are parked, not executable.** `preview_ready` / `preview_pushed`
  blog rows came from the old manual preview/PR workflow. They are not a worker queue and should not
  be counted as blog capacity pressure. Reconcile them one at a time: pick the single best candidate,
  verify the draft/source still exists and fits current strategy, then either (a) convert that one row
  to `approved` so the `blog_content` worker publishes directly to main on its next tick, or (b) set it
  `skipped` / `needs_review` with an evidence note explaining why it is parked. Never bulk-approve the
  preview backlog, never recreate a manual merge gate, and do not create new `preview_ready` blog tasks.
- **NEVER create work that has no finishing executor.** A page-rewrite/indexability task marked as a
  generic `high_risk` ops task has no handler Ã¢â‚¬â€ the safe executor produces nothing and the high-risk
...
- **Do NOT call the executors yourself** (`safe-fix` / `high-risk`) Ã¢â‚¬â€ that double-executes.
...
- **Internal-link tasks need executor-ready link evidence before approval.** Before creating or
  promoting an `internal_link_opportunity` as a deterministic safe/semi-safe ops task, verify that
  `evidence.links` contains one object per insertion with: `source_url` or `source_file` (the page/file
  being edited), `anchor_text`, `to_url`, and `proof_existing_unlinked_mention` (a short quote/snippet
  or line reference proving the exact anchor already appears as plain, currently unlinked text on the
  source page). The safe-fix executor converts existing unlinked mentions into links; it does not write
  new sentences or infer anchors from strategy notes. If you cannot prove an existing unlinked mention,
  do **not** approve a deterministic `internal_link_opportunity`; create a content-lane `content_refresh`
  / `money_page_refresh` / research task with the linking goal in the brief instead.
- **Hold a task** by leaving it `candidate` Ã¢â‚¬â€ workers skip it.
- **Opt-out approval (negative consent).** Reversible safe / semi-safe / high-risk is pre-approved;
  the owner stops/modifies via **Telegram** (`stop <id>`, `change <id> to ...`, `pause`) Ã¢â‚¬â€ always honor that.
- **Hard stop for irreversible work.** Types in `require_explicit_approval` (`config/guardrails.json`)
  must **never** be set `approved`. Start high-risk Phase 1 (`v2 high-risk --task <id>`) Ã¢â€ â€™ holds them in
...
## Step 2: Read the Intelligence Summary  Ã¢â€ Â replaces old Steps 2Ã¢â‚¬â€œ4 (health, GSC fetch, SERP)
...
queue health, and Ã¢â‚¬â€ when recently run Ã¢â‚¬â€ content gaps, competitor moves, money-keyword trends,
...
- **Act on `threats` first** (critical Ã¢â€ â€™ warning). A critical money-keyword drop may warrant a
  `ranking-emergency` task or an immediate fix.
- **Mine `opportunities`** for quick wins, content gaps, and new-page candidates.
- **Note `stale_modules`** Ã¢â‚¬â€ if a module that should have run is missing, plan with that blind spot
...
> This is a bounded, registry-scoped drill-down Ã¢â‚¬â€ not a return to full data-gathering. Only use
...
```bash
cat /opt/website-agent/cron/feedback/latest.md
```
...
- Has the campaign achieved its success metric? Ã¢â€ â€™ mark `completed`.
- Has 60+ days passed without progress? Ã¢â€ â€™ mark `paused` for review.
...
- The **priority ordering** for the next 12 hours, within capacity (ops ~1 task/7 min,
  blog ~1/19 min Ã¢â‚¬â€ do not approve more than the workers can clear).

Apply judgment frameworks:
- **Business Value Assessment** Ã¢â‚¬â€ Revenue (40%) Ã‚Â· Effort (25%, lower=better) Ã‚Â· Momentum (15%) Ã‚Â·
  Competitive difficulty (20%, lower=better). Composite Ã¢â€°Â¥ 3.5 = high, 2.5Ã¢â‚¬â€œ3.5 = medium, <2.5 defer.
- **BIRD** for prioritizing the existing queue Ã¢â‚¬â€ see [task-triage.md](task-triage.md).
- **Cross-reference memory** Ã¢â‚¬â€ don't repeat tactics a Lesson says failed; respect standing Decisions.

### Deduplication rules (before creating)
- Open/in-progress task already covers it Ã¢â€ â€™ **update** it with the fresh signal, don't duplicate.
- Completed <14 days ago and the issue persists Ã¢â€ â€™ wait (changes may still be propagating) or
  reference the old task in a new one only if it clearly regressed.
- Materially different signal (new keyword, new drop) Ã¢â€ â€™ a new task is justified.
- Backlog already >25 open items Ã¢â€ â€™ be selective; create only for money-keyword (Bucket 1) items.

### Homepage is the canonical target for the head money terms (owner decision 2026-06-05)
The **homepage** (`https://example.com/`) is the ranking target for: `website seo`,
`small business owners seo`, `website seo agency`, `small business owners seo agency`, `website seo services`, `seo for small business owners`
(see `config/guardrails.json` Ã¢â€ â€™ `keyword_strategy`). **Do NOT plan dedicated `/services/*` pages for
these** Ã¢â‚¬â€ that cannibalizes the homepage. Support the homepage instead:
- `risk=safe` `internal_link_opportunity` tasks linking these anchors from topically-relevant blogs Ã¢â€ â€™ homepage.
...
```bash
--type new_blog_post \
--evidence '{"blog_cannibalization_check":{"recommendation":"create_new_blog","top_match_url":"...","top_match_score":0.08,"support_url":"https://example.com/"}}'
```

### Money keywords (Bucket 1 Ã¢â‚¬â€ highest priority)
Service intent ("website SEO", "SEO for small business owners", "small business owners SEO services", "website owner SEO"),
pricing/buying ("website SEO pricing/cost/packages"), local ("local SEO for small business owners",
"Google Maps SEO small business owners"), problem-aware ("small business owners needs more customers", "small business owners website not
ranking"). Rule of thumb: if the searcher could become a paying client in 1Ã¢â‚¬â€œ2 conversations, it's a
money keyword. (Bucket 2 = authority/content; Bucket 3 = informational/long-tail; Bucket 4 = noise Ã¢â‚¬â€
homeowner DIY queries and other companies' brand terms Ã¢â‚¬â€ ignore.)
...
```bash
# Quick win on a money keyword (positions 4Ã¢â‚¬â€œ10)
node $V2 task create \
  --title "Quick Win: 'website SEO services' position 7 Ã¢â€ â€™ top 3" \
  --type content_optimization --priority 800 --risk-level safe \
  --target-url "https://example.com/services/website-seo/" \
  --target-keyword "website SEO services" \
...
node $V2 task create \
  --title "THREAT: 'SEO for small business owners' 3 Ã¢â€ â€™ 8" \
  --type content_optimization --priority 900 --risk-level semi_safe \
  --target-url "https://example.com/services/seo-for-small business owners/" \
  --target-keyword "SEO for small business owners" \
  --description "Per threat-detection report (critical): 28d pos 3.1 Ã¢â€ â€™ 7d 8.4, clicks -73%. Investigate algo/competitor/technical." \
...
# Irreversible/destructive types ONLY Ã¢â‚¬â€ hold for explicit approval (never set 'approved')
node $V2 high-risk --task <id> $DB --json   # Phase 1 Ã¢â€ â€™ waiting_for_approval; list under needs_explicit_go
```

> When creating tasks, cite the source report id in `--evidence` so the lineage from
> intelligence Ã¢â€ â€™ task is traceable.

### Substrate repair `target_files` approval gate

Before approving any agent-substrate repair task, enforce this gate for these task types:
`self_improvement`, `process_update`, `prompt_update`, `cron_repair`, `executor_repair`,
and `db_reconciliation`.

- The task evidence must contain exactly one canonical repo-relative
  `evidence.target_files` list before the task is set to `approved`.
- Every `evidence.target_files` entry must be under an allowed substrate path:
  `cli/`, `cron/`, `processes/`, `hermes/skills/client/`, or
  `processes/brain-seed/`.
- Do not approve the repair if `evidence.target_files` is missing, empty, ambiguous,
  absolute, a directory/glob, or outside the allowed substrate paths.
- If the exact target files are unknown, leave the repair as `candidate` and add a note
  describing the missing target-file discovery instead of approving it.
- Do not use this substrate gate to create content or ops work; it only applies to
  planner-approved repairs of the agent substrate.

Example evidence shape for an approved substrate repair:

```json
{
  "evidence": {
    "audit": "audit-<STAMP>",
    "gap": "approved_substrate_repair_missing_target_files",
    "target_files": ["processes/daily-workplan.md"],
    "acceptance": "daily planner blocks approved substrate repairs without target_files"
  }
}
```
...
node $V2 report format --template workplan --format html --json --data '{
  "session": "morning",
  "window": "8:00 AM - 8:00 PM BST (<date>)",
  ...
  "ranking_alerts": [{"keyword":"website seo pricing","detail":"4 Ã¢â€ â€™ 9 over 7 days (threat-detection)"}],
  "notes": "Intelligence: <overall severity>; stale modules: <...>."
}'

node $V2 email send --to owner@example.com \
  --subject "Ã¢Ëœâ‚¬Ã¯Â¸Â Website Operations Work Plan Ã¢â‚¬â€ Morning (<date>)" \
...
per-task notes Ã¢â‚¬â€ the mirror already captures task state. One rollup is enough.
...
**Always create a task if:** a money keyword is outside positions 1Ã¢â‚¬â€œ3 and has a clear action; a
money keyword dropped Ã¢â€°Â¥2 positions in 7 days; a relevant query with >50 imp/wk has no page; a new
competitor entered the top 3 on a money keyword.

**Consider:** Bucket 2 keyword >100 imp/wk in positions 5Ã¢â‚¬â€œ15; a content gap supporting a money
...
**Signal vs noise:** Ã‚Â±2 single-day position moves = noise. 7-day average moves of Ã‚Â±3 on a
...
intelligence summary Ã¢â‚¬â€ you are reading conclusions, not raw numbers.)
